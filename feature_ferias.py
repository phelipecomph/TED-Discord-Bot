...
def schedule_ferias(scheduler,clx):
  ...
  global client
  client = clx
  scheduler.add_job(send_ferias,  CronTrigger(hour="17", minute="00", second="00")) 
...
def get_ferias_text(days=15):
  ...
  if len(lista)>0:text += 'Hey pessoal! Já se prepararam para as férias desses TEDers?\n'
  ...
  if len(text) == 0 and len(lista)>0: text += 'Hey pessoal já podem comemorar, olha quem ja ja ta voltando de ferias!\n'
